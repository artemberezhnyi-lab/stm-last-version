...
def start():
    ...
    try:
        ...
        port = data.get('port', '/dev/ttyUSB0')  # Default for Raspberry Pi
...
def open_browser():
    try:
        ...
        # For Raspberry Pi, try to open browser or just log
        try:
...
if __name__ == '__main__':
    try:
        ...
        logger.info("Starting PWM Generator application on Raspberry Pi...")
        ...
        app.run(
            host="0.0.0.0",  # Allow external connections on Raspberry Pi
            port=5000,
            debug=False,
            use_reloader=False,
            threaded=True
