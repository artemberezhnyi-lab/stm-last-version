...
document.addEventListener('DOMContentLoaded', () => {
    ...
    offset2.addEventListener('input', function() {
        ...
        if (window.previewActive) {
            const channelCard = offset2.closest('.channel-card');
            ...
            // Optimized for Raspberry Pi 4: use throttled update
            if (!canvas._updateThrottled) {
                canvas._updateThrottled = throttle(() => {
                    if (channelNum === 2 || channelNum === 4) {
                        drawPulseChart1(canvas, pulseWidth, parseFloat(offset2.value) || 0, period, null, reverse);
                    } else {
                        drawPulseChart(canvas, pulseWidth, parseFloat(offset2.value) || 0, period, null, reverse);
                    }
                }, 200);
            }
    ...
    offset4.addEventListener('input', function() {
        ...
        if (window.previewActive) {
            const channelCard = offset4.closest('.channel-card');
            const pulseDisplay = channelCard.querySelector('.pulse-display');
            const channel = pulseDisplay.dataset.channel;
            const canvas = pulseDisplay.querySelector('.pulse-chart');
            const pulseWidthInput = channelCard.querySelector('.pulse-width-input');
            ...
            if (!canvas._updateThrottled) {
                canvas._updateThrottled = throttle(() => {
                    if (channelNum === 2 || channelNum === 4) {
                        drawPulseChart1(canvas, pulseWidth, parseFloat(offset4.value) || 0, period, null, reverse);
                    } else {
                        drawPulseChart(canvas, pulseWidth, parseFloat(offset4.value) || 0, period, null, reverse);
                    }
                }, 200);
            }
            canvas._updateThrottled();
    ...
    document.querySelectorAll('.channel-card').forEach(channelCard => {
        channelCard.querySelector('.pulse-width-input').value = '10';
        channelCard.querySelector('.offset-input').value = '10';
        channelCard.querySelector('.power-input').value = '10';
        channelCard.querySelector('.voltage-input').value = '10';
    });
    ...
    function generateOscilloscopeWaveforms() {
        ...
        // Get values for channels 1 and 3 to calculate minimum offset for channels 2 and 4
        const pw1 = parseFloat(document.querySelector(`.pulse-width-input[data-channel="1"]`).value) || 10;
        const off1 = parseFloat(document.querySelector(`.offset-input[data-channel="1"]`).value) || 10;
        const pw3 = parseFloat(document.querySelector(`.pulse-width-input[data-channel="3"]`).value) || 10;
        const off3 = parseFloat(document.querySelector(`.offset-input[data-channel="3"]`).value) || 10;
        
        // Calculate minimum offsets for channels 2 and 4 (considering interlock)
        const minOffset2 = pw1 + off1 + interlock;
        const minOffset4 = pw3 + off3 + interlock;
        ...
        for (let channel = 1; channel <= 4; channel++) {
            ...
            // Get pulseWidth and offset - use same values for all channels
            ...
            // For channels 2 and 4, ensure offset respects minimum value (pulseWidth1/3 + offset1/3 + interlock)
            // This is critical for correct waveform display when interlock < 10
            if (channel === 2) {
                offset = Math.max(offset, minOffset2);
            } else if (channel === 4) {
                offset = Math.max(offset, minOffset4);
            }
            
            // IMPORTANT: Use the SAME pulseWidth for all channels to ensure equal pulse width
            // The offset may differ due to interlock, but pulseWidth must be identical
            // For display purposes, we use the actual pulseWidth value directly
            
            // Generate square wave data - optimized for Raspberry Pi 4 (4GB RAM)
            // All channels use the same pulseWidth to ensure equal pulse duration
    ...
    function drawOscilloscope() {
        ...
        for (let channel = 1; channel <= 4; channel++) {
            if (oscilloscopeData.channels[channel].enabled) {
    ...
    function drawChannelWaveform(ctx, width, height, channel) {
        ...
        // Vertical offset for each channel to separate them (ordered by color)
        // CH1 (yellow) - top, CH2 (green) - upper middle, CH3 (blue) - lower middle, CH4 (magenta) - bottom
        const channelVerticalOffsets = {
            1: -height * 0.35,  // CH1 - top
            2: -height * 0.12,  // CH2 - upper middle
            3: height * 0.12,   // CH3 - lower middle
            4: height * 0.35    // CH4 - bottom
        };
        const channelVerticalOffset = channelVerticalOffsets[channel] || 0;
    ...
    function drawChannelLabels(ctx, width, height) {
        ...
        const labelSpacing = height / 5;
        
        for (let channel = 1; channel <= 4; channel++) {
            if (!oscilloscopeData.channels[channel].enabled) continue;
